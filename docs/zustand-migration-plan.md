...
- [x] **Context removal:** `SeatingChartProvider` / `useSeatingChart` removed; [`SeatingChartEditorView.tsx`](../src/components/dashboard/SeatingChartEditorView.tsx) and [`SeatingEditorLeftNav.tsx`](../src/components/layout/SeatingEditorLeftNav.tsx) read the same fields from `useSeatingStore`. `SeatingChartContext.tsx` deleted; [`useSeatingChartEditor`](../src/hooks/useSeatingChart.ts) unchanged aside from comment.
