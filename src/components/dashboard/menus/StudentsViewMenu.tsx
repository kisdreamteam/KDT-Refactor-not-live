...
export default function StudentsViewMenu({ isOpen, currentView, onViewChange }: StudentsViewMenuProps) {
  ...
  return (
    <div className="absolute bottom-full left-0 z-[100] mb-2 min-w-[200px] rounded-lg border-4 border-brand-purple bg-blue-100 py-2 shadow-lg">
      <div className="border-b border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700">View mode:</div>
      <button
        onClick={() => onViewChange('grid')}
        className={`w-full px-4 py-2 text-left text-sm transition-colors hover:bg-gray-100 ${
          currentView === 'grid' ? 'bg-purple-50 font-medium text-purple-600' : 'text-gray-700'
        }`}
      >
        Student Grid
      </button>
      <button
        ...
        className={`w-full px-4 py-2 text-left text-sm transition-colors hover:bg-gray-100 ${
          currentView === 'seating' ? 'bg-purple-50 font-medium text-purple-600' : 'text-gray-700'
        }`}
      >
        Seating Chart
      </button>
