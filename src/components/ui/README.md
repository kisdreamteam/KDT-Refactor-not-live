...
- Supabase or `@/lib/api/*` imports (Layer 3 belongs in `/src/lib/api` only; see `docs/architecture-plan.md` §3).
- Feature-specific orchestration logic.
- Routing or URL synchronization logic.

## Rules
- Do not import `@/lib/api/*`, `createClient`, or `@supabase/*` from this folder.
...
- Should not import from `src/components/dashboard` (Tier 2); keep this folder Tier 3 + shared primitives only.
